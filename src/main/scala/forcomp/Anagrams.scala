...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences =
    val wLCase = w.toLowerCase
    wLCase.groupBy(c => c).map((c,s) => (c, s.length)).toList.sorted
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] =
    Dictionary.loadDictionary.groupBy(wordOccurrences)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    def updateMap(map: Map[Char, Int], t: List[(Char, Int)]): Map[Char, Int] = t match {
      case Nil => map
      case x :: xs => updateMap(map.updated(x._1, map(x._1) - x._2), xs)
    }
    updateMap(x.toMap, y).toList.filter((c,i) => i != 0).sorted
  }
